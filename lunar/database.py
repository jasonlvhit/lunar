...
class ForeignKeyReverseField(object):
    ...
    def all(self):
        return (
            self.from_class.select("*").where("=".join([self.re, str(self.id)])).all()
        )

    def count(self):
        return (
            self.from_class.select("*").where("=".join([self.re, str(self.id)])).count()
        )
...
class ManyToManyField(object):
    ...
    def _select(self):
        ...
        c = self.db.execute(
            "select %s from %s where %s = %s;" % (rel_id, self.rel, self_id, self.id)
        )
        rs = c.fetchall()
        if len(rs) == 0:
            return self.db.__tabledict__[self.to_table].select().where("id=-1")
        return (
            self.db.__tabledict__[self.to_table]
            .select()
            .where(" or ".join(['id = "%s"' % c[0] for c in rs]))
        )
    ...
    def remove(self, *args):
        ...
        c = self.db.execute(
            ...
            % (self.rel, self.self_id, self.id, " and ".join(list(args))),
            commit=True,
        )
        return c
...
class MetaModel(type):
    def __new__(cls, name, bases, attrs):
        ...
        for name, attr in cls.__dict__.items():
            if isinstance(attr, ForeignKeyReverseField) or isinstance(
                attr, ManyToManyField
            ):
...
class Sqlite(Database):
    ...
    def add(self, instance):
        ...
        cursor.execute(
            ...
            % (instance.__class__.__tablename__, ", ".join(cofk), ", ".join(cofv))
        )
        return cursor
...
class SelectQuery(BaseQuery):
    ...
    def sql(self):
        if self.like_pattern:
            return self.base_statement % (
                ", ".join([str(i).strip("'").strip('"') for i in self.query]).strip(
                    ","
                ),
                self.klass.__tablename__,
                self.like_pattern,
            )
        return self.base_statement % (
            ", ".join([str(i).strip("'").strip('"') for i in self.query]).strip(","),
    ...
    def _make_instance(self, descriptor, r):
        ...
        for _, rf in ins.__dict__.items():
            if isinstance(rf, ManyToManyField) or isinstance(
                rf, ForeignKeyReverseField
            ):
                rf.id = ins.id
        return ins
    ...
    def like(self, pattern):
        ...
        self.base_statement = "".join([self.base_statement.strip(";"), ' like "%s";'])
        return self
...
class UpdateQuery(BaseQuery):
    ...
    def sql(self):
        if self.where:
            self.base_statement = " ".join(["update %s set %s", self.where, ";"])
