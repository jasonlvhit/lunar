...
class BaseTests(unittest.TestCase):
    ...
    def test_dict_2(self):
        rendered = Template("{{ a.get('hello') }}").render(a={"hello": "lunar"})
        self.assertEqual(rendered, "lunar")
...
class FunctionTest(unittest.TestCase):
    ...
    def test_lambda(self):
        rendered = Template("{{ list(map(lambda x: x * 2, [1, 2, 3])) }}").render()
...
class SubtemplateTest(unittest.TestCase):
    def test_block_unmatched(self):
        self.assertRaises(
            TemplateException, Template, "{% block body %}<p>This block body</p>"
        )

    def test_invalid_endblock(self):
        self.assertRaises(TemplateException, Template, "Hello {% endblock %}")

    def test_extend_no_path(self):
        self.assertRaises(
            TemplateException,
            Template,
            """{% extends 'layout.html' %}{% block head %}Hello{% endblock %}'""",
        )

    def test_extends(self):
        rendered = (
            Loader(os.path.dirname(os.path.realpath(__file__)))
            .load("test_extends.html")
            .render(title="lunar")
        )
        self.assertEqual(
            rendered,
